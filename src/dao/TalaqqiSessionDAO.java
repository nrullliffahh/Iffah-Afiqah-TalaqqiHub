...
public class TalaqqiSessionDAO {
    ...
    private static void applyResolvedQuranReference(TalaqqiSession ts,
            ...
            int displaySurah, int displayAyah, int displayAyahEnd, int displayJuzuk) {
        ...
        if (displaySurah > 0) {
            ...
            resolvedAyahEnd = dbAyahEnd > 0 ? dbAyahEnd : 0;
        }

        ts.setCurrentSurahNumber(resolvedSurah);
        ts.setCurrentAyahNumber(resolvedAyah);
        ts.setCurrentAyahEnd(resolvedAyahEnd);
    ...
    /** Live Quran reference for student polling (direct read, same resolution rules as mapRow). */
    public static final class LiveQuranRef {
        public final int surah;
        public final int ayah;
        public final int ayahEnd;
        public final int juzuk;

        public LiveQuranRef(int surah, int ayah, int ayahEnd, int juzuk) {
            this.surah = surah;
            this.ayah = ayah;
    ...
    public LiveQuranRef getLiveQuranReference(String sessionId) {
    ...
    public boolean saveQuranDisplay(String sessionId, int surah, int ayah, int ayahEnd, int juzuk) {
        ...
        try {
            ...
            util.TalaqqiSchemaUtil.ensureQuranDisplayAyahEndColumn(conn);
     ...
     * Booking status becomes Completed only when the student attended (Present/Late).
     * Absent / no-show bookings stay active so Class Booking shows Not Completed + Reschedule.
     * 
     * @param sessionId The session ID
    ...
    public boolean completeSession(String sessionId, String teacherId) {
        ...
        try {
            ...
            String attendedExpr = sqlBookingStudentAttended();
            String bookingStatusExpr = "cb.bookingStatus = IF(" + attendedExpr + ", 'Completed', cb.bookingStatus)";
            String updateSql;
            if (usesBookingIdLink(conn) && hasSessionTimingColumns(conn)) {
                ...
                updateSql =
                    ...
                    + joinSessionToBooking(conn)
                    + "JOIN classschedule cs ON cb.scheduleId = cs.scheduleId "
                    + "SET " + bookingStatusExpr + ", ts.sessionDate = CURDATE(), cs.classStatus = 'Completed' "
                    + "WHERE ts.sessionId = ? AND cs.teacherId = ?";
            } else {
                updateSql =
                    "UPDATE classbooking cb "
            ...
            if (rowsUpdated <= 0) {
                if (completeSessionByResolvedBooking(sessionId, teacherId, durationMinutes)) {
                    rowsUpdated = 1;
                }
            }
    ...
    /**
     * Marks booking Completed when teacher and student both joined the live session.
     * Called when the student leaves after a conducted session.
     */
    public boolean completeSessionIfConducted(String sessionId, String teacherId) {
        if (sessionId == null || sessionId.trim().isEmpty()) {
            return false;
        }
        if (!hasConductedSession(sessionId.trim())) {
            return false;
        }
        if (isBookingCompletedForSession(sessionId.trim())) {
            return true;
        }
    ...
    public boolean hasConductedSession(String sessionId) {
        ...
        try {
            ...
            if (!hasStudentJoinedSession(conn, sid)) {
                return false;
            }
            if (LIVE_SESSION_START_EPOCH.containsKey(sid)) {
                return true;
            }
            if (!hasSessionTimingColumns(conn)) {
                return false;
            ...
            try (PreparedStatement ps = conn.prepareStatement(
                    "SELECT 1 FROM " + sessionTable + " WHERE sessionId = ? AND sessionStartTime IS NOT NULL LIMIT 1")) {
                ps.setString(1, sid);
                try (ResultSet rs = ps.executeQuery()) {
                    return rs.next();
                }
            ...
            System.err.println("[TalaqqiSessionDAO] hasConductedSession: " + e.getMessage());
            return false;
        } finally {
            if (conn != null) {
                try {
    ...
    private boolean isBookingCompletedForSession(String sessionId) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            String sql =
                "SELECT cb.bookingStatus FROM classbooking cb "
                + joinSessionToBooking(conn)
                + "WHERE ts.sessionId = ? LIMIT 1";
            ps = conn.prepareStatement(util.TalaqqiSchemaUtil.sql(sql, conn));
            ps.setString(1, sessionId);
    ...
    /** Direct booking/session/schedule update when JOIN-based completeSession matches no row. */
    private boolean completeSessionByResolvedBooking(String sessionId, String teacherId, double durationMinutes) {
        ...
        if (teacherId != null && session.getTeacherId() != null
                && !teacherId.equals(session.getTeacherId())) {
            return false;
        }
        ...
        try {
            ...
            int bookingRows = 0;
            if (bookingId != null && !bookingId.trim().isEmpty()) {
                if (bookingStudentAttended(conn, bookingId.trim())) {
                    try (PreparedStatement ps = conn.prepareStatement(
                            "UPDATE classbooking SET bookingStatus = 'Completed' "
                                + "WHERE bookingId = ? AND bookingStatus NOT IN ('Cancelled','Rescheduled')")) {
                        ps.setString(1, bookingId.trim());
                        bookingRows = ps.executeUpdate();
                    }
                }
            }
            if (bookingRows == 0 && scheduleId != null && studentId != null
                    && bookingStudentAttended(conn, scheduleId, studentId)) {
                try (PreparedStatement ps = conn.prepareStatement(
                        "UPDATE classbooking SET bookingStatus = 'Completed' "
            ...
            if (hasSessionTimingColumns(conn)) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(
                        "UPDATE " + sessionTable + " SET sessionDate = CURDATE() WHERE sessionId = ?")) {
                    ps.setString(1, sessionId);
                    ps.executeUpdate();
                }
            }
            ...
            if (bookingRows > 0) {
                backfillSessionBookingId(conn, sessionId);
                System.out.println("[TalaqqiSessionDAO] completeSessionByResolvedBooking: sessionId="
                    + sessionId + ", bookingId=" + bookingId + ", scheduleId=" + scheduleId);
            }
            return true;
        } catch (SQLException e) {
            System.err.println("[TalaqqiSessionDAO] completeSessionByResolvedBooking: " + e.getMessage());
            return false;
        } finally {
            if (conn != null) {
                try {
    ...
    private String getScheduleIdBySessionId(String sessionId) {
        ...
        try {
            ...
            String sql = usesBookingIdLink(conn)
                ? "SELECT COALESCE(cb.scheduleId, ts.scheduleId) AS scheduleId "
                    + "FROM talaqqisession ts "
                    + "LEFT JOIN classbooking cb ON "
    ...
    private TalaqqiSession mapRow(ResultSet rs) throws SQLException {
        ...
        try {
            Object ayahEndObj = rs.getObject("displayAyahEnd");
            if (ayahEndObj instanceof Number) {
                displayAyahEnd = ((Number) ayahEndObj).intValue();
            }
        } catch (SQLException ignored) {}
        try {
        ...
        applyResolvedQuranReference(ts, dbSurah, dbAyah, dbAyahEnd,
            displaySurah, displayAyah, displayAyahEnd, displayJuzuk);
     ...
     * Late if more than 5 minutes after teacher start. Uses Malaysia timezone.
     */
    public String determineAttendanceStatus(String sessionId, String studentId, Time joinTime) {
        ...
        LocalDateTime studentJoin = LocalDateTime.of(LocalDate.now(zone), joinTime.toLocalTime());

        LocalDateTime sessionStart = resolveTeacherLiveSessionStart(sessionId);
        if (sessionStart == null) {
            System.out.println("[TalaqqiSessionDAO] determineAttendanceStatus: teacher has not started "
                + sessionId + " — student waiting, marking Present");
            return "Present";
        }
        ...
        System.out.println("[TalaqqiSessionDAO] Student " + studentId
                + " joined within " + LATE_THRESHOLD_MINUTES + " min of teacher start — Present");
        return "Present";
    }
    ...
    private LocalDateTime resolveTeacherLiveSessionStart(String sessionId) {
        ...
        if (epoch != null) {
            return LocalDateTime.ofInstant(
                java.time.Instant.ofEpochMilli(epoch), util.AppTimeUtil.APP_ZONE);
        }
        return null;
    }
    ...
    private LocalDateTime getSessionStartDateTime(String sessionId) {
        ...
        try {
            ...
            if (startTs != null) {
                return startTs.toInstant().atZone(util.AppTimeUtil.APP_ZONE).toLocalDateTime();
            }
    ...
    private boolean bookingStudentAttended(Connection conn, String bookingId) throws SQLException {
        String sql = "SELECT 1 FROM classbooking cb WHERE cb.bookingId = ? AND " + sqlBookingStudentAttended();
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    private boolean bookingStudentAttended(Connection conn, String scheduleId, String studentId) throws SQLException {
        String sql =
            "SELECT 1 FROM classbooking cb "
            + "WHERE cb.scheduleId = ? AND cb.studentId = ? AND " + sqlBookingStudentAttended()
            + " LIMIT 1";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, scheduleId);
            ps.setString(2, studentId);
